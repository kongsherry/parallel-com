...
inline uint32x4_t F_neon(uint32x4_t x, uint32x4_t y, uint32x4_t z) {
	...
	uint32x4_t notx = vmvnq_u32(x);
	uint32x4_t notx_z = vandq_u32(notx, z);
	return vorrq_u32(xy, notx_z);
}

inline uint32x4_t G_neon(uint32x4_t x, uint32x4_t y, uint32x4_t z) {
	uint32x4_t xz = vandq_u32(x, z);
	uint32x4_t notz = vmvnq_u32(z);
	uint32x4_t y_notz = vandq_u32(y, notz);
	return vorrq_u32(xz, y_notz);
}

inline uint32x4_t H_neon(uint32x4_t x, uint32x4_t y, uint32x4_t z) {
	uint32x4_t xy = veorq_u32(x, y);
	return veorq_u32(xy, z);
}

inline uint32x4_t I_neon(uint32x4_t x, uint32x4_t y, uint32x4_t z) {
	uint32x4_t notz = vmvnq_u32(z);
	uint32x4_t x_or_notz = vorrq_u32(x, notz);
