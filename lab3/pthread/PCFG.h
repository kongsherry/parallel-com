...
public:
    ...
    vector<string> guesses;
     // 添加线程安全相关成员
    pthread_mutex_t guess_mutex;  // 保护guesses数组的互斥锁
    std::mutex cout_mutex;        // 线程安全打印
    
    
    // 线程参数结构体
    struct ThreadArgs {
        PriorityQueue* queue;      // 指向队列的指针
        segment* seg;             // 指向当前segment的指针
        PT* pt;                   // 当前PT结构
        string prefix;            // 前缀字符串(用于多segment情况)
        int start_idx;            // 起始索引
        int end_idx;              // 结束索引
    };
    
    // 线程工作函数声明
    static void* GenerateThread(void* args);
    // 添加线程池成员
    ThreadPool* pool;
