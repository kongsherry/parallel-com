...
void* PriorityQueue::GenerateThread(void* args) {
    ThreadArgs* targs = (ThreadArgs*)args;
    PriorityQueue* q = targs->queue;
    segment* a = targs->seg;
    PT* pt = targs->pt;
    string prefix = targs->prefix;
    ...
    pthread_mutex_lock(&q->guess_mutex);
    q->guesses.insert(q->guesses.end(), local_guesses.begin(), local_guesses.end());
    q->total_guesses += local_guesses.size();
    pthread_mutex_unlock(&q->guess_mutex);
...
void PriorityQueue::Generate(PT pt)
{
    ...
    if (pt.content.size() == 1)
    {
        ...
        for (int i = 0; i < num_threads; i++) {
            ...
            args->end_idx = (i == num_threads-1) ? total : (i+1)*chunk_size;
            
            pool->addTask(GenerateThread, args);
        }
