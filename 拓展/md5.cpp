...
void MD5Hash_SIMD(string input[4], uint32x4_t* state)
{
	...
	for (int i = 0; i < n_blocks; i += 1)
	{
		...
		for (int i1 = 0; i1 < 16; ++i1)
		{
			uint32_t val0 = (paddedMessage[0][4 * i1 + i * 64]) |
				(paddedMessage[0][4 * i1 + 1 + i * 64] << 8) |
				(paddedMessage[0][4 * i1 + 2 + i * 64] << 16) |
				(paddedMessage[0][4 * i1 + 3 + i * 64] << 24);
			uint32_t val1 = (paddedMessage[1][4 * i1 + i * 64]) |
				(paddedMessage[1][4 * i1 + 1 + i * 64] << 8) |
				(paddedMessage[1][4 * i1 + 2 + i * 64] << 16) |
				(paddedMessage[1][4 * i1 + 3 + i * 64] << 24);
			uint32_t val2 = (paddedMessage[2][4 * i1 + i * 64]) |
				(paddedMessage[2][4 * i1 + 1 + i * 64] << 8) |
				(paddedMessage[2][4 * i1 + 2 + i * 64] << 16) |
				(paddedMessage[2][4 * i1 + 3 + i * 64] << 24);
			uint32_t val3 = (paddedMessage[3][4 * i1 + i * 64]) |
				(paddedMessage[3][4 * i1 + 1 + i * 64] << 8) |
				(paddedMessage[3][4 * i1 + 2 + i * 64] << 16) |
				(paddedMessage[3][4 * i1 + 3 + i * 64] << 24);
			
			x[i1] = vsetq_lane_u32(val0, x[i1], 0);
			x[i1] = vsetq_lane_u32(val1, x[i1], 1);
			x[i1] = vsetq_lane_u32(val2, x[i1], 2);
			x[i1] = vsetq_lane_u32(val3, x[i1], 3);
	...
	for (int i = 0; i < 4; i++)
	{
		uint8x16_t bytes = vreinterpretq_u8_u32(state[i]);
		uint8x16_t swapped_bytes = vrev32q_u8(bytes);
		state[i] = vreinterpretq_u32_u8(swapped_bytes);
	}
